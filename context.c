...
context_t *g_context_defaults(context_t *ctx) {
	...
	if (err != ESP_OK) {
		ELOG(TAG, "Get base MAC address from BLK3 of EFUSE error (%s)",
			 esp_err_to_name(err));
	}
...
uint16_t semVer() { return (uint16_t)LOGGER_VERSION; }
uint8_t semVerMajor() { return (uint8_t)VERSION_MAJOR; }
uint8_t semVerMinor() { return (uint8_t)VERSION_MINOR; }
uint8_t semVerPatch() { return (uint8_t)VERSION_PATCH; }
uint8_t semVerBuild() { return (uint8_t)VERSION_TWEAK; }
